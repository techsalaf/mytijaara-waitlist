...
export const Route = createFileRoute("/admin/users")({
  head: () => ({ meta: [{ title: "Users — MyTijaara Admin" }, { name: "robots", content: "noindex" }] }),
  component: UsersPage,
});

function UsersPage() {
  const [q, setQ] = useState("");
  const filtered = adminUsers.filter((u) => (u.name + u.email + u.role).toLowerCase().includes(q.toLowerCase()));
  return (
      ...
      <SectionCard>
        <div className="mb-4 flex gap-2">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search team…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
          </div>
              ...
              {filtered.map((u) => (
                <tr key={u.id} className="border-t border-border/40">
                  ...
                  <td className="py-3 text-right">
                    <Button asChild variant="ghost" size="sm"><Link to="/admin/users/$id" params={{ id: u.id }}>View</Link></Button>
                  </td>
                </tr>
              ))}
            </tbody>
