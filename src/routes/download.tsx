...
type DownloadCmsData = {
  ...
  subheading?: string;
  /**
    * Store buttons carry no `label`: "GET IT ON / Google Play" and "Download on
    * the App Store" are fixed by the Google Play and Apple badge guidelines, so
    * the page renders them verbatim and the CMS deliberately cannot change them.
    */
  playStore?: {
    enabled?: boolean;
    comingSoon?: boolean;
  ...
  /**
   * The "What you can do with MyTijaara" grid. Seeded since day one and, until
   * now, rendered by nobody: the page mapped over a hardcoded `APP_FEATURES`
   * array, so editing the cards was impossible and the seeded rows sat unread in
   * the database. `icon` is a registry name, resolved by `resolveContentIcon`.
   */
  features?: {
    icon?: string;
    title?: string;
...
/**
 * Bundled copy for the feature grid, used when the CMS row is empty.
 *
 * Shape matches `download.features` in the database exactly (`icon` is a
 * `CONTENT_ICONS` name, not a component), so the fallback and the CMS path go
 * through one renderer instead of two.
 */
const APP_FEATURES: NonNullable<DownloadCmsData["features"]> = [
  {
...
const DOWNLOAD_FAQS = [
  ...
  },
  {
    ...
    a: `${DEFAULT_LAUNCH_CITY} is our first launch city, with ${PHASE_TWO_CITIES.join(", ")} next and a fast ongoing rollout to other Nigerian states.`,
  },
  {
...
function DownloadPage() {
  ...
  // filtered length: switching every card off is a deliberate choice and must
  // render nothing, not quietly restore the six bundled cards.
  const savedFeatures =
    downloadCms.features && downloadCms.features.length > 0 ? downloadCms.features : APP_FEATURES;
  const features = savedFeatures.filter((feature) => feature.enabled !== false);
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {features.map((feature, index) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl border ${tint}`}>
