...
export const Route = createFileRoute("/admin/audit-logs")({
  head: () => ({ meta: [{ title: "Audit Logs — MyTijaara Admin" }, { name: "robots", content: "noindex" }] }),
  component: () => (
    <div className="space-y-6">
      <PageHeader
        title="Audit Logs"
        description="Every important action across your admin panel."
        actions={<Button size="sm" variant="outline"><Download className="mr-2 h-4 w-4" /> Export</Button>}
      />
      <SectionCard>
        ...
        <ol className="relative space-y-4 border-l-2 border-border/60 pl-6">
          {activityLog.map((e) => (
            <li key={e.id} className="relative rounded-xl border border-border/60 bg-white p-4">
              <span className="absolute -left-[30px] top-4 grid h-4 w-4 place-items-center rounded-full border-2 border-primary bg-white">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="secondary" className="text-xs">{e.target}</Badge>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1">
                  {e.device.includes("Server") ? <Server className="h-3 w-3" /> : <Smartphone className="h-3 w-3" />} {e.device}
                </span>
              </div>
            </li>
          ))}
