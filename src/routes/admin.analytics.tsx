...
function Analytics() {
  ...
  const [signupTrend, setSignupTrend] = useState<SignupTrendPoint[]>([]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      analyticsApi.browsers(),
      analyticsApi.cities(),
      analyticsApi.funnel(),
    ]).then((responses) => {
      if (!active) return;
      setStats(responses[0].data);
      setSignupTrend(responses[1].data);
      setDeviceBreakdown(responses[2].data);
      setBrowserBreakdown(responses[3].data);
      setCityBreakdown(responses[4].data);
      setFunnel(responses[5].data);
    });
    return () => {
      active = false;
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          label="CTA clicks"
          value={(stats?.ctaClicks ?? 0).toLocaleString()}
          delta={0}
          icon={MousePointerClick}
        />
      </div>

      <SectionCard title="Signups & verified users (30 days)">
        <div className="h-72">
          ...
          <div className="space-y-3">
            {cityBreakdown.map((c) => (
              <div key={c.city} className="flex items-center justify-between text-sm">
                ...
                <span className="text-muted-foreground">
                  {c.users} · <span className="text-emerald-600">+{c.growth}%</span>
                </span>
              </div>
            ))}
          </div>
