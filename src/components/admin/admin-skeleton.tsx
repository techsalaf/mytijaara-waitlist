...
export function AdminSkeleton() {
  return (
    <div className="flex min-h-screen bg-[#F8FAF8]">
      <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 border-r border-border/60 bg-white">
        ...
        <main className="p-4 lg:p-8">
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-28 animate-pulse rounded-2xl border border-border/60 bg-white" />
            ))}
          </div>
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            <div className="h-80 animate-pulse rounded-2xl border border-border/60 bg-white lg:col-span-2" />
            <div className="h-80 animate-pulse rounded-2xl border border-border/60 bg-white" />
          </div>
        </main>
      </div>
    </div>
