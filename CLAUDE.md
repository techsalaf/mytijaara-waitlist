...
The marginal cost of completeness is near zero with AI. Do the whole thing. Do it right. Do it with tests. Do it with documentation. Do it so well that Rasheed is genuinely impressed — not politely satisfied, actually impressed. Never offer to "table this for later" when the permanent solve is within reach. Never leave a dangling thread when tying it off takes five more minutes. Never present a workaround when the real fix exists. The standard isn't "good enough" — it's "holy shit, that's done."

Search before building. Test before shipping. Ship the complete thing. When Rasheed asks for something, the answer is the finished product, not a plan to build it.
...
- When the software we build needs to call an LLM, do NOT use an LLM API (Anthropic API, OpenAI API, any hosted inference endpoint) unless Rasheed explicitly instructs it. Route the call through the local Claude Code instead.
...
- Always use the best available model by default unless Rasheed explicitly instructs otherwise. No silent downgrades to a cheaper or smaller model for cost.

### Tech choice — Laravel by default

- Simplest Laravel tech wins. No framework-of-the-month. No clever abstractions for hypothetical reuse.
...
- If two options are equally viable, name the trade-off explicitly and ask Rasheed. Confusion Protocol applies.
...
- **DONE_WITH_CONCERNS** — Completed, but with issues Rasheed should know about. List each concern with severity and a proposed follow-up.
...
2. **Report what to restart.** Tell Rasheed exactly which service / system / program needs to be restarted for the change to take effect, with the full list of commands to run. If nothing needs restarting, say so explicitly.

For restart commands that need `sudo`: never run them yourself. List them for Rasheed to run, clearly marked as his to execute.
...
STOP. Name the ambiguity in one sentence. Present 2-3 options with real trade-offs (not a fake spread). Ask Rasheed. Do not guess on architectural decisions. Does not apply to routine coding, small features, or obvious changes.
...
## How Rasheed wants to be talked to
...
When Rasheed asks for something, the answer is the finished product — not a plan. Tests included. Evals included. Docs included.
...
### Database Operations
- `pnpm dlx drizzle-kit generate` - Generate migration scripts from schema changes
- `pnpm dlx drizzle-kit migrate` - Apply pending migrations to the database
- `pnpm dlx drizzle-kit studio` - Open Drizzle Studio to visualize and edit data

### Testing
- No test framework is configured in this repository yet
- For manual testing, use the development server and test API endpoints directly
...
- **Email**: Resend (optional, configured via environment variables)
- **Utilities**: Zod (validation), Nanoid (ID generation), Class Variance Authority
...
/app
  /api
    ...
    /waitlist
      /join            # Waitlist signup (POST) and count (GET)
      /referrals/[code] # Referral lookup
    /admin
...
/components
  countdown-timer.tsx  # Countdown to launch event
  waitlist-counter.tsx # Live waitlist member count
  waitlist-form.tsx    # Multi-step waitlist form
  /ui                  # Shadcn/ui component wrappers

/lib
  /auth                # Better Auth configuration
  /db
    index.ts           # Database connection (Drizzle + MySQL2)
...
Modified from original PostgreSQL schema for MySQL compatibility:
- Primary keys: `varchar(255)` instead of `text`
- Arrays: `json` type instead of `text[]`
- Timestamps: Proper `timestamp`/`datetime` usage with `defaultNow()`
- Key tables: `waitlist_entries`, `referrals`, `email_events`, `activity_logs`, plus auth tables

### Current Issues & Troubleshooting

#### Database Connection Issues
The application is currently experiencing database connection errors (500 responses from API routes). To resolve:
...
2. **Check credentials in .env.local**:
   ```
   DATABASE_URL="mysql://root@localhost:3306/mytijaara"
...
3. **Test database connection**:
   ```bash
   mysql -u root -h localhost -P 3306 -D mytijaara
   ```
...
#### API Route Debugging
When encountering 500 errors from `/api/waitlist/join`:
1. Check server console for detailed error logs
2. Verify database connectivity in `lib/db/index.ts`
3. Ensure `.env.local` changes are picked up (restart dev server)
4. Confirm MySQL user has appropriate privileges
...
- Leverage Server Actions for data mutations where appropriate

### Database Changes
1. All database interactions should go through the Drizzle ORM client in `lib/db/index.ts`
2. Use parameterized queries to prevent SQL injection
3. Handle database errors gracefully with try/catch blocks
4. Return appropriate HTTP status codes in API routes

### API Routes
- All API routes are in `/app/api/` and use the Next.js Route Handlers
- Use `NextResponse.json()` for JSON responses
- Validate all inputs with Zod schemas
- Handle errors with appropriate status codes (400 for client errors, 500 for server errors)

### Environment Management
- Environment variables are stored in `.env.local`
- Never commit sensitive information to version control
- Use `process.env` to access environment variables securely

## Key Files to Understand First
1. `package.json` - Dependencies and scripts
2. `.env.local` - Environment configuration (especially DATABASE_URL)
3. `lib/db/index.ts` - Database connection setup
4. `lib/db/schema.ts` - Database schema definition
5. `app/api/waitlist/join/route.ts` - Main waitlist API endpoint
