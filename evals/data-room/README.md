...
question the gate lane cannot: **does anything a stranger or a partially
authorized visitor receives from this API say something it should not?**
...
**1. Capture the corpus.** Drives the real API and writes every visitor-facing
payload to `captures/`.
...
30 cases as of the last capture. The set is chosen so that the leaks worth
...
| Path | What it is |
| --- | --- |
| `rubric.md` | The three dimensions, the verdict format, the thresholds. The judge's entire instruction set. |
...
- **Non-determinism is bounded, not removed.** The same corpus judged twice can
  ...
  threshold and the two security dimensions are absolute.
- **Admin-facing payloads are not captured.** The corpus covers what an outsider or
  a visitor can obtain, because that is where a leak reaches someone who should not
  see it. An admin payload that over-shares is a different, lower-severity problem
  and is left to the gate lane's structural assertions.
