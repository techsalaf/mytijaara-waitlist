    ...
    public function publicSettings(): JsonResponse
    {
        ...
        return response()->json([
            'data' => [
                ...
                'tagline'         => $co['tagline']      ?? 'Everything you need, all in one place',
                // Company contact fields used by footer, email templates, etc.
                'contactEmail'    => $co['contactEmail'] ?? 'hello@mytijaara.com',
                'supportEmail'    => $co['supportEmail'] ?? 'support@mytijaara.com',
                'phone'           => $co['phone']        ?? '+2349032617923',
                'launchCity'      => $co['launchCity']   ?? 'Ibadan',
                'address'         => $co['address']      ?? '',
                'logoUrl'         => $br['logoUrl']         ?? '',
                'logoDarkUrl'     => $br['logoDarkUrl']     ?? '',
                'faviconUrl'      => $br['faviconUrl']      ?? '',
                'ogImageUrl'      => $br['ogImageUrl']      ?? '',
                'primaryColor'    => $br['primaryColor']    ?? '#004A28',
                'accentColor'     => $br['accentColor']     ?? '#D4A017',
                'secondaryColor'  => $br['secondaryColor']  ?? '#166534',
                'backgroundColor' => $br['backgroundColor'] ?? '#F8FAF8',
                'surfaceColor'    => $br['surfaceColor']    ?? '#FFFFFF',
                // Social links are public — no secrets involved.
                'social' => [
                    'instagram' => $so['instagram'] ?? 'https://instagram.com/mytijaara',
                    'twitter'   => $so['twitter']   ?? 'https://x.com/mytijaara',
                    'facebook'  => $so['facebook']  ?? 'https://facebook.com/mytijaara',
                    'linkedin'  => $so['linkedin']  ?? '',
                    'tiktok'    => $so['tiktok']    ?? 'https://tiktok.com/@mytijaara',
                    'youtube'   => $so['youtube']   ?? '',
                    'whatsapp'  => $so['whatsapp']  ?? 'https://whatsapp.com/channel/0029VbE63oZGOj9vfKqWbQ1D',
                ],
            ],
        ]);
    }
    ...
    private function rules(string $group): array
    {
        return match ($group) {
            ...
            'branding' => [
                ...
                'secondaryColor'  => ['sometimes', 'nullable', 'string', 'max:32'],
                'backgroundColor' => ['sometimes', 'nullable', 'string', 'max:32'],
                'surfaceColor'    => ['sometimes', 'nullable', 'string', 'max:32'],
            ],
            'seo' => [
